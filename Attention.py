...
def train(model, node_features, adj, num_epochs, saved = False, num_classes = 3, lambda_reg =0.5):
    ...
    for epoch in range(num_epochs):
        ...
        if saved:
            if (epoch +1) % 100 ==0:
                file_path = "./models/"
                torch.save(model, f"{file_path}new_attention_{epoch}_model_withmask.pt")
